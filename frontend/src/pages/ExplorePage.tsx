...
import { fetchViewport, setDepth } from '../store/exploreSlice'
...
function toVisData(viewport: any) {
  const nodes = viewport.nodes.map((n: any): VisNetworkNode => ({
    id: n.uid,
...
export default function ExplorePage({ selectedUid, onSelectUid }: ExplorePageProps) {
  ...
  const { viewport, depth, loading, error } = useSelector((state: RootState) => state.explore)
  ...
  useEffect(() => {
    dispatch(fetchViewport({ uid: selectedUid, depth }))
  }, [selectedUid, depth, dispatch])

  useEffect(() => {
    const el = containerRef.current
    if (!el || !visData) return

    const nodes = new DataSet<VisNetworkNode>(visData.nodes)
    const edges = new DataSet<VisNetworkEdge>(visData.edges)
    ...
    const network = new Network(el, { nodes, edges }, {
      ...
      nodes: {
        ...
        borderWidth: 1,
        color: {
          border: 'rgba(255,255,255,0.25)', background: 'rgba(124, 92, 255, 0.35)',
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div className="kb-panel" style={{ flex: 1, borderRadius: 18, position: 'relative', overflow: 'hidden' }}>
        <div style={{
          ...
          background: 'radial-gradient(800px 500px at 50% 40%, rgba(124, 92, 255, 0.18), transparent 60%), linear-gradient(180deg, rgba(255,255,255,0.02), rgba(255,255,255,0))',
        }} />
